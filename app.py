...
def nome():
    ...
    return render_template('index.html',selected_nome=nome, nomes=nomes)
...
def execute_query2(assetids , datainicio, datafim, cliente):
    ...
    # Verificar se o arquivo de cache existe para o cliente e nome especificados
    cache_file = os.path.join(client_folder, f'{cliente}_{datainicio[:10]}.pkl')
    if os.path.exists(cache_file):
        # Carregar dados do cache a partir do arquivo
        ...
        query2 = f'''SELECT trip.assetid,
        asset.description,
        trip.tripstart,
        trip.tripend,
        asset.assetimageurl
        FROM ita_mix.trip as trip
        join "ita_mix"."asset_to_garage"  as asset on CAST(asset.assetid AS VARCHAR)= trip.assetid
        {where_clause}
        AND (trip.tripstart >= ('{datainicio}'))
        AND (trip.tripstart <= ('{datafim}')) 
        and trip.tripstart is not null
        and trip.tripend is not null 
        ORDER BY trip.tripstart;'''
        print(query2)
...
def veiculos():
    ...
    return render_template('index.html',assetids=assetids2,selected_data = dia, descricao = assetplaca, selected_nome=nome, nomes=nomes, dia=dia, variavelcontrole1='ok')
...
def selectdata():
        ...
        #datainicio = df1['tripstart'].min()
        #print(datainicio)
        return render_template('index.html',descricao=assetplaca, selected_nome= nome,nomes=nomes,datas1= datas_organizadas1, datas2= datas_organizadas2, selected_data = dia, selected_assetid=assetid, variavelcontrole1='ok')
